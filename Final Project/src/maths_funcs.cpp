...
// This is my own for calculating Euler angles from a direction vector
float ydegs(vec3 directionVector) {
	directionVector = normalise(directionVector);

	// acos won't give signed angle.
	float angle;
	if (directionVector.v[2] > 0) {
		angle = -std::acos(directionVector.v[0]) * 180.0f / M_PI;
	}
	else {
		angle = std::acos(directionVector.v[0]) * 180.0f / M_PI;
	}
